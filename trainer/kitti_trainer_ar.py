...
from utils.flow_utils import load_flow, evaluate_flow, np_resize_flow, flow_to_image
...
class TrainFramework(BaseTrainer):
    ...
    def _run_one_epoch(self):
        ...
        if 'loss_stage' in self.cfg:
            if self.i_iter == self.cfg.loss_stage.iter:
        ...
        for i_set, loader in enumerate(self.valid_loader):
            ...
            for i_step, data in enumerate(loader):
                ...
                if self.cfg.dump_en and i_step % self.cfg.dump_freq == 0:
                    dump_results(((gt_flows[0][:,:,2] - gt_flows[0][:,:,3])*255).astype(np.uint8), val_idx=i_set, samp_idx=i_step, res=es[0], samp_type="occ",path=self.save_root)
                    ...
                    err_mag = [np.sqrt(((flo_p - flo_gt)**2).sum(axis=2)) for flo_p,flo_gt in zip(pred_flows,gt_flows[0])]
                    err_vec = [flow_to_image(flo_p - flo_gt) for flo_p,flo_gt in zip(pred_flows,gt_flows[0])]
