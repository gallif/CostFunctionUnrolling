...
def smooth_grad_1st(flo, image, alpha, mode='abs'):
    ...
    if mode == 'abs':
        ...
        loss_x = weights_x * F.smooth_l1_loss(dx,torch.zeros_like(dx), reduction='none') / 2.
        loss_y = weights_y * F.smooth_l1_loss(dy,torch.zeros_like(dy), reduction='none') / 2.
    elif mode == 'charbonnier':
        loss_x = weights_x * (dx**2 + 0.01**2).sqrt() / 2.
        loss_y = weights_y * (dy**2 + 0.01**2).sqrt() / 2.
    else:
...
def smooth_grad_2nd(flo, image, alpha, mode='abs'):
    img_dx, img_dy = gradient(image)
    weights_x = torch.exp(-torch.mean(torch.abs(img_dx), 1, keepdim=True) * alpha)
    weights_y = torch.exp(-torch.mean(torch.abs(img_dy), 1, keepdim=True) * alpha)
    ...
    if mode == 'abs':
        ...
        loss_x = weights_x[:, :, :, 1:] * F.smooth_l1_loss(dx2,torch.zeros_like(dx2), reduction='none') / 2.
        loss_y = weights_y[:, :, 1:, :] * F.smooth_l1_loss(dy2,torch.zeros_like(dy2), reduction='none') / 2.
    elif mode == 'charbonnier':
        loss_x = weights_x[:, :, :, 1:] * (dx2**2 + 0.01**2).sqrt() / 2.
        loss_y = weights_y[:, :, 1:, :] * (dy2**2 + 0.01**2).sqrt() / 2.
    else:
        raise(NotImplementedError(mode))
