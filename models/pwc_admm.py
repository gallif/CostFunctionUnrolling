...
class PWCLite(nn.Module):
    def __init__(self, cfg):
        ...
        self.admm_solver = cfg.admm_solver
        ...
        if cfg.admm_solver:
            self.admm_block = ADMMSolverBlock(rho=cfg.admm_args.rho, lamb=cfg.admm_args.lamb, eta=cfg.admm_args.eta, 
                grad=cfg.admm_args.grad, T=cfg.admm_args.T)
            self.mask_gen = MaskGenerator(alpha=cfg.admm_args.alpha)
            self.apply_admm = cfg.admm_args.apply_admm
    ...
    def forward_2_frames(self, x1_pyramid, x2_pyramid, mask):
        ...
        for l, (x1, x2) in enumerate(zip(x1_pyramid, x2_pyramid)):
            ...
            # Apply ADMM Solver
            if self.admm_solver and self.apply_admm[l]:
    ...
    def forward(self, x, with_bk=False):
        ...
        if self.admm_solver:
            masks = [self.mask_gen(img, scale=1/4) for img in imgs]
        else:
            masks = [None]*len(imgs) 

        res_dict = {}
        if n_frames == 2:
            res_dict['flows_fw'] = self.forward_2_frames(x[0], x[1], masks[0])
            if with_bk:
                res_dict['flows_bw'] = self.forward_2_frames(x[1], x[0], masks[1])
        else:
